...
pub fn eliminate_phi_block(block: &[nodes::Ssa], mut added_end: Vec<nodes::Ssa>) -> Vec<nodes::Ssa> {
    let mut out = vec![];
    ...
    for (idx, b) in block.iter().enumerate() {
        ...
        if idx == block.len() - 1 {
            ...
            match b {
                ...
                nodes::Ssa::Jump(_) |
                nodes::Ssa::Branch { cond: _, true_target: _, false_target: _ } => {
    ...
    for b in blocks.iter() {
        ...
        for p in phis {
            ...
            for merge in p.merging {
                added_end_assgn.entry(merge.1.to_ir_string()).or_default()
                    .push(
                        ir::nodes::Ssa::Assignment { dest: dest.clone(), source: merge.0, width: p.width }
                    );
            }
        }
    }
