use crate::{opt::phi_elim::eliminate_phi_body, ir::nodes};
    ...
    unit.iter().map(|u| {
        match u {
            nodes::ToplevelItem::Function(f) => {
                ...
                nodes::ToplevelItem::Function( nodes::FunctionDef { name: f.name.clone(),
                    return_width: f.return_width.clone(), parameters: f.parameters.clone(),
                    body:  elim_phi
                 })
            },
