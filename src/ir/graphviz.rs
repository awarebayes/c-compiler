...
pub fn graphviz_function(declaration: &nodes::FunctionDef) -> String
{
    ...
    for block in blocks.iter() {
        ...
        let body = block.iter().skip(1).map(|x| html_escape::encode_safe_to_string(&x.to_ir_string(), &mut String::new()).to_owned()).collect::<Vec<_>>().join(sep);
        res.push_str(&body);
        res.push_str(sep);
        res.push('>');
        res.push(']');
    ...
    for block in blocks.iter() {
        ...
        for i in block.iter().skip(1) {
            match i {
                ...
                nodes::Ssa::Branch { cond: _, true_target, false_target } => {
                    res.push_str(&format!("{} -> {}[label=\"true\"];\n", lab_ir, true_target.to_ir_string()));
                    res.push_str(&format!("{} -> {}[label=\"false\"];\n", lab_ir, false_target.to_ir_string()));
                    ended_jump = true;
                },
                _ => ()
            }
